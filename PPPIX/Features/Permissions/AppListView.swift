...
struct AppListView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                if manager.isLoadingApps {
                    Spacer()
                    ProgressView().tint(Color(hex: "#3366FF")).scaleEffect(1.4)
                    Spacer()
                } else {
                    ScrollView {
        ...
        .confirmationDialog(
            ...
            Text("Remova também o perfil em Configurações → Geral → VPN e Gerenciamento.")
        }
    }
}
...
private struct AppRow: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            AppIconView(name: app.name, size: 46)
            ...
            if isBlocked {
                HStack(spacing: 4) {
                    ...
                    Text("Protegido").font(.caption.bold())
                }
                .foregroundColor(Color(hex: "#44FF88"))
...
private struct BlockedChip: View {
    ...
    var body: some View {
        HStack(spacing: 6) {
            AppIconView(name: app.name, size: 26)
            Text(app.name).font(.caption.bold()).foregroundColor(.white).lineLimit(1)
            Button(action: onRemove) {
                Image(systemName: "xmark.circle.fill")
                    .foregroundColor(Color(white: 0.45)).font(.system(size: 13))
            }
        }
        .padding(.horizontal, 10).padding(.vertical, 6)
        .background(Color(white: 0.09))
        .cornerRadius(20)
        .overlay(RoundedRectangle(cornerRadius: 20).stroke(Color(hex: "#44FF88").opacity(0.25), lineWidth: 1))
    }
}

// MARK: — Ícone com iniciais e cor baseada no nome
struct AppIconView: View {
    ...
    private var color: Color {
        let colors: [Color] = [
            Color(hex: "#FF6B35"), Color(hex: "#F7C59F"), Color(hex: "#EFEFD0"),
            Color(hex: "#004E89"), Color(hex: "#1A936F"), Color(hex: "#C3423F"),
            Color(hex: "#9BC4CB"), Color(hex: "#F4D35E"), Color(hex: "#EE964B"),
            Color(hex: "#3A86FF"), Color(hex: "#8338EC"), Color(hex: "#FF006E"),
        ]
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                VStack(spacing: 24) {
                    AppIconView(name: app.name, size: 76).padding(.top, 8)
                    ...
                    if step == 0 {
                        VStack(spacing: 14) {
                            StepInfo(icon: "apps.iphone", color: "#3366FF",
                                     title: "Atalho idêntico criado",
                                     desc: "Um atalho com o nome \(app.name) aparece na tela inicial")
                        ...
                        VStack(spacing: 12) {
                            Button { activate() } label: {
                                Group {
                                    ...
                                    else { Text("Ativar Proteção").font(.system(size: 16, weight: .semibold)).foregroundColor(.white) }
                                }
                        ...
                        VStack(spacing: 16) {
                            Image(systemName: "checkmark.shield.fill")
                                .font(.system(size: 60)).foregroundColor(Color(hex: "#44FF88"))
                            Text("Quase lá!")
                                .font(.title3.bold()).foregroundColor(.white)
                        ...
                        Button("Concluído") { isPresented = false }
                            ...
                            .frame(maxWidth: .infinity).frame(height: 52)
                            .background(Color(hex: "#44FF88")).cornerRadius(13)
...
private struct StepInfo: View {
    let icon: String; let color: String; let title: String; let desc: String
    var body: some View {
        HStack(alignment: .top, spacing: 14) {
            Image(systemName: icon).font(.system(size: 18)).foregroundColor(Color(hex: color))
                .frame(width: 38, height: 38).background(Color(hex: color).opacity(0.1)).cornerRadius(10)
            VStack(alignment: .leading, spacing: 2) {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            Text(n).font(.caption.bold()).foregroundColor(.white)
                .frame(width: 20, height: 20).background(Color(hex: "#3366FF")).clipShape(Circle())
